...
type (
	...
	KeyAuthValidator func(string, echo.Context) (bool,*AuthContext, error)
...
func CustomKeyAuth(fn KeyAuthValidator) echo.MiddlewareFunc {
	c := DefaultKeyAuthConfig
	c.Validator = fn
	return CustomKeyAuthWithConfig(c)
}

// KeyAuthWithConfig returns an KeyAuth middleware with config.
// See `KeyAuth()`.
func CustomKeyAuthWithConfig(config KeyAuthConfig) echo.MiddlewareFunc {
	// Defaults
		...
		return func(next echo.HandlerFunc) echo.HandlerFunc {
			return func(c echo.Context) error {
				...
				return &echo.HTTPError{
					...
					Internal: fmt.Errorf("missing validator"),
				}
			}
		}
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			valid,authContext, err := config.Validator(key, c)
			if err != nil {
